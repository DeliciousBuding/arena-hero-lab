...
/** 场景 × 条目热图：资源/杀率/存活 三选切换，纯 SVG 渲染（无图表库） */
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center gap-3">
        <div className="inline-flex rounded-xl border border-border-primary bg-surface-tertiary/60 p-1">
          {METRICS.map((m) => (
            <button
              key={m.key}
              type="button"
              ...
              className={`rounded-lg px-3 py-1.5 text-xs font-medium transition-colors ${
                metricKey === m.key
                  ? "bg-surface-primary text-accent-primary shadow-card"
      ...
      <div className="card thin-scroll overflow-x-auto p-2">
        <svg
            ...
            return (
              <g key={entry.contestantId}>
                ...
                {contestant?.kind === "builtin" ? (
                  <circle cx={x} cy={HEADER_H - 4} r={2.5} fill="var(--rank-gold)" />
                ) : null}
              </g>
            );
          })}
                    ...
                    <g key={entry.contestantId}>
                      ...
                      {v != null ? (
                        <text
                          ...
                          textAnchor="middle"
                          className="tnum"
                          style={{ fontSize: 12.5, fontWeight: 600, fill: `var(--heat-text-${step})` }}
                        >
                          {fmt(v)}
                        </text>
                      ) : null}
                      <text
                        ...
                        y={cy + 13}
                        textAnchor="middle"
                        style={{ fontSize: 9.5, fill: `var(--heat-text-${step})`, opacity: 0.75 }}
                      >
          ...
          <g>
            <defs>
              <linearGradient id="heat-legend" x1="0" y1="0" x2="1" y2="0">
                {[1, 2, 3, 4, 5, 6].map((s) => (
                  <stop key={s} offset={`${((s - 1) / 5) * 100}%`} stopColor={`var(--heat-${s})`} />
                ))}
                <stop offset="100%" stopColor="var(--heat-6)" />
              </linearGradient>
            </defs>
            <text x={ROW_LABEL_W + PAD} y={height - LEGEND_H + 6} style={{ fontSize: 10, fill: "var(--text-tertiary)" }}>
              低
            </text>
            <rect
            ...
            <text x={width - PAD} y={height - LEGEND_H + 6} textAnchor="end" style={{ fontSize: 10, fill: "var(--text-tertiary)" }}>
              单元格悬浮查看详情 · 列头为总体名次
            </text>
          </g>
        </svg>
